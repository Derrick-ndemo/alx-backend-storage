...
+ [x] 2. **Best band ever!**<br/>[2-fans.sql](2-fans.sql) contains a SQL script that ranks country origins of bands, ordered by the number of (non-unique) fans.
  ...
  + Your script can be executed on any dat script that creates a trigger that decreases the quantity of an item after adding a new order:
...
+ [x] 4. **Buy buy buy**<br/>[4-store.sql](4-store.sql) contains a SQL
